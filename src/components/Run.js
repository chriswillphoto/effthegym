      ...
      return (
        <div className="run-container">
          ...
          {this.state.pauseTimer ? (
            <button className="time-button" onClick={() => this.setState({ pauseTimer: false })}>Go</button>
          ) : (
            <button className="time-button" onClick={() => this.setState({ pauseTimer: true })}>Pause</button>
          )}
        </div>
      );
    } else {
